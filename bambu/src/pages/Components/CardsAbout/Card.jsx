...
const Card = ({ id, image, front, back }) => {
  ...
  return (
    <div
      key={'a' + id}
      className="w-80 h-60 md:w-80 lg:w-96 mx-auto my-4 transition-all duration-500 ease-in-out transform hover:scale-105 flex-1 text-center block rounded-lg bg-transparent bg-opacity-50 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] dark:bg-neutral-700"
      onMouseEnter={handleMouseEnter}
      onMouseLeave={handleMouseLeave}
    >
      {show ? (
        <span key={'c' + id} className="max-w-full h-64 bg-green-500 flex rounded-md items-center justify-center">
          <p key={'bt' + id} className="mx-4 text-xl text-white animated animate__fadeIn">
            {back}
          </p>
        </span>
      ) : (
